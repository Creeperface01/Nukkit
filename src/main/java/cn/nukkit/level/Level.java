...
public class Level implements ChunkManager, Metadatable {
    ...
    private static Function<Level, ChunkRequestManager> chunkRequestManagerFactory = DefaultChunkRequestManager::new;
    ...
    private final Int2IntMap loaderCounter = new Int2IntArrayMap();
    ...
    private Long2ObjectMap<List<DataPacket>> chunkPackets = new Long2ObjectOpenHashMap<>();

    private final Long2LongMap unloadQueue = new Long2LongArrayMap();
    ...
    private Long2ObjectMap<SoftReference<Short2ObjectMap<Object>>> changedBlocks = new Long2ObjectOpenHashMap<>();
    ...
    private final Short2ObjectMap<Object> changeBlocksFullMap = new Short2ObjectOpenHashMap<Object>() {
        @Override
        public int size() {
            return 32768;
        }
...
//    private final List<BlockUpdateEntry> nextTickUpdates = Lists.newArrayList();
    ...
    private Long2IntMap chunkTickList = new Long2IntOpenHashMap();
    private int chunksPerTicks;
    private boolean clearChunksOnTick;
    private final Int2ObjectMap<Class<? extends Block>> randomTickBlocks = new Int2ObjectOpenHashMap<Class<? extends Block>>() {
        {
    ...
    public final ChunkRequestManager chunkRequestManager = chunkRequestManagerFactory.apply(this);
    ...
    public void registerChunkLoader(ChunkLoader loader, int chunkX, int chunkZ, boolean autoLoad) {
        ...
        long index = Level.chunkHash(chunkX, chunkZ);
        if (!this.chunkLoaders.containsKey(index)) {
            this.chunkLoaders.put(index, new Int2ObjectOpenHashMap<>());
            this.playerLoaders.put(index, new Int2ObjectOpenHashMap<>());
...
//        this.timings.entityTick.stopTiming();
        ...
        if (!this.changedBlocks.isEmpty()) {
            if (!this.players.isEmpty()) {
                for (Map.Entry<Long, SoftReference<Short2ObjectMap<Object>>> entry : changedBlocks.entrySet()) {
                    long index = entry.getKey();
                    Map<Short, Object> blocks = entry.getValue().get();
                    this.chunkRequestManager.onChunkChanged(index);
                ...
                this.chunkRequestManager.clearCache(true);
            }

            this.changedBlocks.clear();
        }

        this.chunkRequestManager.process();
    ...
    public void clearCache(boolean full) {
        this.chunkRequestManager.clearCache(full);
    }

    public void clearChunkCache(int chunkX, int chunkZ) {
        this.chunkRequestManager.onChunkChanged(chunkX, chunkZ);
    }
            ...
            if (currentTick % 600 == 0) { //30 seconds
                ...
                while (entityIterator.hasNext()) {
                    ...
                    for (ChunkLoader loader : this.loaders.values()) {
                        if (entity.distanceSquared(loader.getPosition()) <= tickRadius) {
                            close = true;
                            break;
                        }
                    }
...
//        }
    ...
    public boolean setBlock(Vector3 pos, Block block, boolean direct, boolean update) {
        ...
        if (this.getChunk((int) pos.x >> 4, (int) pos.z >> 4, true).setBlock((int) pos.x & 0x0f, (int) pos.y & 0xff,
            ...
            if (direct) {
                this.sendBlocks(this.getChunkPlayers((int) position.x >> 4, (int) position.z >> 4).values().toArray(new Player[0]), new Block[]{block}, UpdateBlockPacket.FLAG_ALL_PRIORITY);
                this.chunkRequestManager.onChunkChanged(index);
            } else {
    ...
    private void addBlockChange(long index, int x, int y, int z) {
        SoftReference<Short2ObjectMap<Object>> current = changedBlocks.computeIfAbsent(index, k -> new SoftReference(new Short2ObjectOpenHashMap<>()));
    ...
    public void requestChunk(int x, int z, Player player) {
        ...
        this.chunkRequestManager.requestChunk(index, player);
    }
    ...
    public static void setChunkRequestManagerFactory(Function<Level, ChunkRequestManager> chunkRequestManagerFactory) {
        Level.chunkRequestManagerFactory = chunkRequestManagerFactory;
    }
    ...
    private static ChunkCacher chunkCacher = new ChunkCacher() {
        @Override
        public ChunkRequestTask.Result getData(int x, int z, byte[] payload, byte[] data11) {
            ...
            return new ChunkRequestTask.Result(batch, batch, x, z);
        }
    };
    ...
    public static ChunkRequestTask.Result getChunkCacheFromData(int x, int z, byte[] data, byte[] data11) {
        return chunkCacher.getData(x, z, data, data11);
    }

    public interface ChunkCacher {

        ChunkRequestTask.Result getData(int x, int z, byte[] payload, byte[] data11);
    }
}
